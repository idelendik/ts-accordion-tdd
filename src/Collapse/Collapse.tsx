...
export const ACCORDION_MODE_ERROR =
  "<Collapse /> only accept 1 only item in `accordion` mode";
...
export function Collapse({
  ...
  const onHeaderClick = (name: string) => {
    const isOpened = openItemsSet.has(name);
  ...
  return (
    <CollapseProvider value={{ openedItems: openItemsSet, onHeaderClick }}>
      {childrenArray}
    </CollapseProvider>
  );
}
